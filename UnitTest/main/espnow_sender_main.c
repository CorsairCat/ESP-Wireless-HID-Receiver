...
static uint8_t temp_broadcast_mac[ESP_NOW_ETH_ALEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
...
void espnow_recv_data_prepare(espnow_send_param_t *send_param)
{
    ...
    if (s_espnow_seq[buf->type] > 910) {s_espnow_seq[buf->type] = 0;}
    buf->crc = 0;
...
static void espnow_send_task(void *pvParameter)
{
    ...
    while (xQueueReceive(s_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
        switch (evt.id) {
            ...
            {
                espnow_event_send_cb_t *send_cb = &evt.info.send_cb;
                // buf->type = (send_param->state < ESP_RECV_STATE_CONNECTED) ? ESPNOW_DATA_BROADCAST : ESPNOW_DATA_UNICAST;

                if (system_stage < ESP_RECV_STATE_CONNECTED && system_stage != send_cb->stage /*|| time out */ ) {
                    break;
                }
                
                if (system_stage >= ESP_RECV_STATE_CONFIRM) {
                    // send unicast data
                }
            ...
            case ESPNOW_RECV_CB:
            {
                ...
                if (ret == ESPNOW_DATA_BROADCAST) {
                    ...
                    if (system_stage == ESP_RECV_STATE_AWAITING && recv_magic == send_param->magic && recv_state == ESP_RECV_STATE_RESPONDING){
                ...
                else if (ret == ESPNOW_DATA_UNICAST && system_stage >= ESP_RECV_STATE_RESPONDING) {
                    
                    ESP_LOGI(TAG, "Receive %dth unicast data from: "MACSTR", len: %d", recv_seq, MAC2STR(recv_cb->mac_addr), recv_cb->data_len);

                    if (system_stage == ESP_RECV_STATE_RESPONDING && recv_state == ESP_RECV_STATE_CONFIRM && esp_now_is_peer_exist(recv_cb->mac_addr))
                    {
                        system_stage = ESP_RECV_STATE_CONNECTED;
                        send_param->state = system_stage;
                    ...
                    else if (system_stage == ESP_RECV_STATE_CONFIRM) {system_stage = ESP_RECV_STATE_CONNECTED;}
...
static esp_err_t espnow_receiver_init(void)
{
    ...
    xTaskCreate(espnow_send_task, "espnow_task", 4096, send_param, 4, NULL);
